...
# FFMpeg & General timeout constants
...
async def safe_task_wrapper(coro):
    try:
        ...
        task_name = asyncio.current_task().get_name()
        logger.exception(f"Unhandled exception in background task: {task_name}")
...
def get_progress_markup():
    return InlineKeyboardMarkup([[InlineKeyboardButton("❌ ᴄᴀɴᴄᴇʟ", callback_data="cancel_upload")]])
...
async def safe_edit_message(message, text, reply_markup=None, parse_mode=enums.ParseMode.MARKDOWN):
    try:
        ...
        # Check if new text is different from the current one to avoid "MESSAGE_NOT_MODIFIED" error
        if current_text and hasattr(current_text, 'strip') and current_text.strip() == text.strip():
            # If reply_markup is different, still edit
...
async def restart_bot(msg):
    ...
    await msg.reply(
        ...
        "The bot will now shut down cleanly. If running under a process manager "
        "(like Koyeb, Docker, or systemd), it will restart automatically."
    )
    # This will cause app.run() to exit, allowing the script to terminate.
    await app.stop()
...
async def show_stats(_, msg):
    ...
    for p in PREMIUM_PLATFORMS:
        group_stage[f"{p}_premium"] = {"$sum": {"$cond": [f"$platforms.{p}", 1, 0]}}
    ...
    if action == "waiting_for_instagram_username":
        ...
        async def login_task():
            try:
                ...
                logger.error(f"ᴜɴʜᴀɴᴅʟᴇᴅ ᴇʀʀᴏʀ ᴅᴜʀɪɴɢ ɪɴꜱᴛᴀɢʀᴀᴍ ʟᴏɢɪɴ ғᴏʀ {user_id} ({username}): {e}", exc_info=True)
            finally:
                if user_id in user_states:
                    del user_states[user_id]
        ...
        try:
            ...
            # The processing_msg object is a Pyrogram Message object which is not BSON-serializable.
            # We must remove it before saving the dictionary to MongoDB.
            processing_msg_object = file_info.pop("processing_msg", None)

            # Now file_info is safe to be stored in the database.
            job_data = {
                "user_id": user_id,
                "file_info": file_info, # This dict is now safe.
                "schedule_time": schedule_time,
                "status": "pending",
                "original_chat_id": msg.chat.id,
                "original_msg_id": msg.id,
                # Store chat/message IDs of the status message so the scheduled job can update it later.
            ...
            # Insert the sanitized job_data into the database.
            await asyncio.to_thread(db.scheduled_posts.insert_one, job_data)

            # Reply to the user using the message object we stored in memory.
            ...
            logger.error(f"Error scheduling post for user {user_id}: {e}", exc_info=True)
        finally:
            if user_id in user_states:
                del user_states[user_id]
        ...
        try:
            ...
            await msg.reply("❌ ɪɴᴠᴀʟɪᴅ ᴜꜱᴇʀ ɪᴅ. ᴩʟᴇᴀꜱᴇ ꜱᴇɴᴅ ᴀ ᴠᴀlid ɴᴜᴍʙᴇʀ.")
            if user_id in user_states:
                del user_states[user_id]
        ...
        try:
            ...
            await msg.reply("❌ ɪɴᴠᴀʟɪᴅ ɪɴᴩᴜᴛ. ᴩʟᴇᴀꜱᴇ ꜱᴇɴᴅ ᴀ ᴠᴀlid ɴᴜᴍʙᴇʀ.")
            if user_id in user_states:
                del user_states[user_id]
        ...
        if not is_admin(user_id): return
        proxy_url = msg.text
        if proxy_url.lower() in ["none", "remove", "clear"]:
        ...
        if msg.reply_to_message:
            await global_settings_panel_cb(None, msg.reply_to_message)
        ...
        # The original message is needed for user context in start_upload_task
        original_msg = msg.reply_to_message.reply_to_message if msg.reply_to_message and msg.reply_to_message.reply_to_message else msg
        await start_upload_task(original_msg, file_info)
...
async def cancel_upload_cb(_, query):
    ...
    # Cancel all background tasks associated with this user first
...
async def skip_caption_cb(_, query):
    ...
    # The original message that started the upload flow is needed for context
    original_msg = query.message.reply_to_message if query.message.reply_to_message else query.message
    if not hasattr(original_msg, 'from_user') or not original_msg.from_user:
        original_msg.from_user = query.from_user
...
async def show_plan_details_cb(_, query):
    ...
    price_multiplier = 1 # This could be dynamic in a future update
    plan_details = PREMIUM_PLANS[plan_key]

    plan_text = f"**{plan_key.replace('_', ' ').title()} ᴩʟᴀɴ ᴅᴇᴛᴀɪʟꜱ**\n\n"
    plan_text += f"**ᴅᴜʀᴀᴛɪᴏɴ**: "
    plan_text += f"{plan_details['duration'].days} ᴅᴀyꜱ\n" if plan_details['duration'] else "ʟɪғᴇᴛɪᴍᴇ\n"
...
async def show_payment_methods_cb(_, query):
    text = "**ᴀᴠᴀɪʟᴀʙʟᴇ ᴩᴀyᴍᴇɴᴛ ᴍᴇᴛʜᴏᴅꜱ**\n\nChoose your preferred method to proceed."
...
async def show_payment_qr_google_play_cb(_, query):
    ...
    if not qr_file_id:
        return await query.answer("ɢᴏᴏɢʟᴇ ᴩᴀy ǫʀ ᴄᴏᴅᴇ ɪꜱ ɴᴏᴛ ꜱᴇᴛ ʙy ᴛʜᴇ ᴀᴅᴍɪɴ yᴇᴛ.", show_alert=True)
...
async def global_settings_panel_cb(_, query_or_msg):
    # This handler can be called by a query or a regular message object
    if isinstance(query_or_msg, InlineKeyboardButton): # Placeholder, needs actual query object
        query = query_or_msg
    ...
    if not is_admin(user_id):
        if query: return await query.answer("❌ Admin access required.", show_alert=True)
        return
...
async def back_to_cb(_, query):
    ...
    # Clear any pending states and tasks when navigating back
    task_tracker.cancel_all_user_tasks(user_id)
    if user_id in user_states:
        del user_states[user_id]
    ...
    if data == "back_to_main_menu":
        ...
        await app.send_message(
            query.message.chat.id, "🏠 ᴍᴀɪɴ ᴍᴇɴᴜ",
        ...
        await admin_panel_cb(_, query)
    elif data == "back_to_premium_plans":
        await buypypremium_cb(_, query)
    else:
...
async def confirm_reset_stats_cb(_, query):
    if not is_admin(query.from_user.id): return await query.answer("❌ Admin access required.", show_alert=True)
    if db is None: return await query.answer("⚠️ Database is unavailable.", show_alert=True)
    ...
    await send_log_to_channel(app, LOG_CHANNEL, f"📊 ᴀᴅᴍɪɴ `{query.from_user.id}` ʜᴀꜱ ʀᴇꜱᴇᴛ ᴀʟʟ ʙᴏᴛ ᴜᴩʟᴏᴀᴅ ꜱᴛᴀᴛɪꜱᴛɪᴄꜱ.")
        ...
        try:
            ...
            if gpus:
                ...
                for i, gpu in enumerate(gpus):
                    gpu_info += (
                        f"      - **ɢᴩᴜ {i}:** `{gpu.name}`\n"
                        f"      - ʟᴏᴀᴅ: `{gpu.load*100:.1f}%` | ᴍᴇᴍᴏʀy: `{gpu.memoryUsed}/{gpu.memoryTotal}` ᴍʙ\n"
                    )
            else:
                gpu_info = "ɴᴏ ɢᴩᴜ ғᴏᴜɴᴅ."
        except Exception:
            gpu_info = "ᴄᴏᴜʟᴅ ɴᴏᴛ ʀᴇᴛʀɪᴇᴠᴇ ɢᴩᴜ ɪɴғᴏ (GPUtil not installed?)."
        
        system_stats_text += gpu_info
        await safe_edit_message(
            query.message, system_stats_text,
            reply_markup=get_admin_global_settings_markup(), parse_mode=enums.ParseMode.MARKDOWN
        )
    except Exception as e:
        await query.answer("❌ ғᴀɪʟᴇᴅ ᴛᴏ ʀᴇᴛʀɪᴇᴠᴇ ꜱyꜱᴛᴇᴍ ꜱᴛᴀᴛꜱ.", show_alert=True)
        logger.error(f"ᴇʀʀᴏʀ ʀᴇᴛʀɪᴇᴠɪɴɢ ꜱyꜱᴛᴇᴍ ꜱᴛᴀᴛꜱ: {e}")
        await safe_edit_message(query.message, "🛠 ᴀᴅᴍɪɴ ᴩᴀɴᴇʟ", reply_markup=admin_markup)
...
async def users_list_cb(_, query):
    ...
    if not users:
        return await safe_edit_message(query.message, "👥 ɴᴏ ᴜꜱᴇʀꜱ ғᴏᴜɴᴅ.", reply_markup=admin_markup)

    user_list_text = "👥 **ᴀʟʟ ᴜꜱᴇʀꜱ:**\n\n"
    for user in users:
        user_id = user["_id"]
        instagram_username = user.get("instagram_username", "ɴ/ᴀ")
        added_at = user.get("added_at", datetime.min).strftime("%Y-%m-%d")
        last_active = user.get("last_active", datetime.min).strftime("%Y-%m-%d %H:%M")
        ...
        user_list_text += (
            ...
            f"ɪɢ: `{instagram_username}` | ʟᴀꜱᴛ ᴀᴄᴛɪᴠᴇ: `{last_active}`\n"
            "-----------------------------------\n"
        )
    ...
    if len(user_list_text) > 4096:
        ...
        await query.message.reply_document("users.txt", caption="👥 ᴀʟʟ ᴜꜱᴇʀꜱ ʟɪꜱᴛ")
        os.remove("users.txt")
        await query.answer("User list sent as a file.")
    else:
        await safe_edit_message(
            query.message, user_list_text,
            reply_markup=admin_markup, parse_mode=enums.ParseMode.MARKDOWN
        )
...
async def select_platform_cb(_, query):
    ...
    state_data = user_states.get(user_id)
    if not (isinstance(state_data, dict) and state_data.get("action") == "select_platforms_for_premium"):
        await query.answer("Session expired. Please start again.", show_alert=True)
        return await admin_panel_cb(_, query)
    ...
    await safe_edit_message(
        query.message,
        f"✅ ᴜꜱᴇʀ ɪᴅ `{state_data['target_user_id']}`. Select platforms for premium:",
...
async def grant_plan_cb(_, query):
    ...
    state_data = user_states.get(user_id)
    if not (isinstance(state_data, dict) and state_data.get("action") == "select_premium_plan_for_platforms"):
        await query.answer("❌ Error: State lost. Please start over.", show_alert=True)
        return await admin_panel_cb(_, query)
    ...
    for platform in selected_platforms:
        premium_data[platform] = {
            "type": premium_plan_key,
            "added_by": user_id,
            "added_at": datetime.utcnow(),
            "until": datetime.utcnow() + plan_details["duration"] if plan_details["duration"] else None
        }
    
    await _save_user_data(target_user_id, {"premium": premium_data})

    admin_confirm_text = f"✅ Premium granted to user `{target_user_id}` for:\n"
    user_msg_text = "🎉 **ᴄᴏɴɢʀᴀᴛᴜʟᴀᴛɪᴏɴꜱ!** 🎉\nYou have been granted premium access:\n"

    for platform in selected_platforms:
        p_data = premium_data[platform]
        confirm_line = f"**{platform.capitalize()}**: `{p_data['type'].replace('_', ' ').title()}`"
        if p_data.get("until"):
            confirm_line += f" (Expires: `{p_data['until'].strftime('%Y-%m-%d %H:%M')}` UTC)"
        else:
             confirm_line += " (Lifetime)"
    ...
    try:
        ...
        log_text = f"💰 Premium granted to `{target_user_id}` by admin `{user_id}`. Plan: `{premium_plan_key}` for `{', '.join(selected_platforms)}`"
...
async def broadcast_message_cb(_, query):
    ...
    await safe_edit_message(
        query.message,
        "📢 To broadcast, use the command: `/broadcast <message>`"
    )
...
async def admin_stats_panel_cb(_, query):
    ...
    stats_text = (
        ...
        f"**ᴛᴏᴛᴀʟ ᴜꜱᴇʀꜱ**: `{total_users}`\n"
        f"**ᴛᴏᴛᴀʟ ᴜᴩʟᴏᴀᴅꜱ**: `{total_uploads}`\n\n"
        "Use `/stats` command for more detailed stats."
    )
...
async def upload_type_cb(_, query):
    await safe_edit_message(query.message, "📌 Select the default upload type:", reply_markup=upload_type_markup)
...
async def set_caption_cb(_, query):
    ...
    await safe_edit_message(query.message, "📝 ᴩʟᴇᴀꜱᴇ ꜱᴇɴᴅ yᴏᴜʀ ɴᴇᴡ ᴅᴇғᴀᴜʟᴛ ᴄᴀᴩᴛɪᴏɴ.")
...
async def set_hashtags_cb(_, query):
    ...
    await safe_edit_message(query.message, "🏷️ Please send your new default hashtags.")

@app.on_callback_query(filters.regex("^set_aspect_ratio$"))
async def set_aspect_ratio_cb(_, query):
    await safe_edit_message(query.message, "📐 Select the aspect ratio for your videos:", reply_markup=aspect_ratio_markup)
...
async def handle_media_upload(_, msg):
    ...
    if not state_data or state_data.get("action") not in ["waiting_for_instagram_reel_video", "waiting_for_instagram_photo_image"]:
    ...
    if not media:
        return await msg.reply("❌ Unsupported media type. Please send a video or photo.")
    ...
    try:
        ...
        logger.error(f"ᴇʀʀᴏʀ ᴅᴜʀɪɴɢ ғɪʟᴇ ᴅᴏᴡɴʟᴏᴀᴅ ғᴏʀ ᴜꜱᴇʀ {user_id}: {e}", exc_info=True)
        await safe_edit_message(file_info.get("processing_msg"), f"❌ ᴅᴏᴡɴʟᴏᴀᴅ ғᴀɪʟᴇᴅ: {e}")
        cleanup_temp_files([file_info.get("downloaded_path")])
        if user_id in user_states: del user_states[user_id]
...
async def process_and_upload(msg, file_info, is_scheduled=False, job_id=None):
    user_id = msg.from_user.id if hasattr(msg, 'from_user') and msg.from_user else file_info['user_id']
    platform = file_info["platform"]
    upload_type = file_info["upload_type"]
    file_path = file_info["downloaded_path"]
    
    processing_msg = file_info.get("processing_msg")
    # If it's a scheduled job, try to reconstruct the message object to edit it
    if is_scheduled and not processing_msg:
        try:
            p_chat_id = file_info.get("processing_chat_id")
            p_msg_id = file_info.get("processing_msg_id")
            if p_chat_id and p_msg_id:
                processing_msg = await app.get_messages(p_chat_id, p_msg_id)
        except Exception as e:
            logger.warning(f"Could not retrieve processing_msg for scheduled job {job_id}: {e}")
    ...
    async with upload_semaphore:
        ...
        try:
            ...
            is_video = any(file_path.lower().endswith(ext) for ext in ['.mp4', '.mkv', '.webm', '.avi', '.mov', '.flv'])
            
            if is_video and not no_compression_admin:
                await safe_edit_message(processing_msg, "🔄 ᴏᴩᴛɪᴍɪᴢɪɴɢ ᴠɪᴅᴇᴏ (ᴛʀᴀɴꜱᴄᴏᴅɪɴɢ)...")
                ...
                try:
                    ...
                    if process.returncode != 0:
                        raise Exception(f"Video transcoding failed. This can happen with corrupted files. FFmpeg error: {stderr.decode()[:500]}")
            ...
            settings = await get_user_settings(user_id)
            final_caption = file_info.get("custom_caption", settings.get("caption", ""))
            if settings.get("hashtags"):
                final_caption += f"\n\n{settings.get('hashtags')}"

            url, media_id, media_type_value = "ɴ/ᴀ", "ɴ/ᴀ", "ɴ/ᴀ"
            await safe_edit_message(processing_msg, f"🚀 **ᴜᴩʟᴏᴀᴅɪɴɢ ᴛᴏ {platform.capitalize()}...**", reply_markup=get_progress_markup())

            if platform == "instagram":
                user_upload_client = InstaClient()
                user_upload_client.delay_range = [1, 3]
                proxy_url = global_settings.get("proxy_url", INSTAGRAM_PROXY)
                if proxy_url: user_upload_client.set_proxy(proxy_url)
                
                session = await load_instagram_session(user_id)
                if not session: raise LoginRequired("Instagram session expired or not found.")
                user_upload_client.set_settings(session)
                
                try:
                    await asyncio.to_thread(user_upload_client.get_timeline_feed)
                except LoginRequired:
                    raise LoginRequired("Instagram session is invalid. Please /login again.")
                ...
                url = f"https://instagram.com/{'reel' if upload_type == 'reel' else 'p'}/{result.code}"
                media_id = result.pk
            ...
            if db is not None:
                await asyncio.to_thread(db.uploads.insert_one, {
                    ...
                    "media_type": str(media_type_value), "platform": platform,
                    "upload_type": upload_type, "timestamp": datetime.utcnow(),
                    "url": url, "caption": final_caption
                })

            log_msg = (
                f"📤 ɴᴇᴡ {platform.capitalize()} {upload_type.capitalize()} ᴜᴩʟᴏᴀᴅ\n"
                f"👤 ᴜꜱᴇʀ: `{user_id}` | 🔗 ᴜʀʟ: {url}"
            )
            await safe_edit_message(processing_msg, f"✅ ᴜᴩʟᴏᴀᴅᴇᴅ ꜱᴜᴄᴄᴇꜱꜱғᴜʟʟy!\n\n{url}")
            await send_log_to_channel(app, LOG_CHANNEL, log_msg)

            if is_scheduled:
                await asyncio.to_thread(db.scheduled_posts.update_one, {"_id": job_id}, {"$set": {"status": "completed"}})

        except Exception as e:
            error_msg = f"❌ {platform.capitalize()} ᴜᴩʟᴏᴀᴅ ғᴀɪʟᴇᴅ: {str(e)}"
            await safe_edit_message(processing_msg, error_msg) if processing_msg else await app.send_message(user_id, error_msg)
            logger.error(f"{platform.capitalize()} upload failed for {user_id}: {e}", exc_info=True)
            if is_scheduled:
                await asyncio.to_thread(db.scheduled_posts.update_one, {"_id": job_id}, {"$set": {"status": "failed", "error": str(e)}})
        finally:
...
async def check_scheduled_posts():
    ...
    now = datetime.utcnow()
    try:
        jobs_cursor = db.scheduled_posts.find({
            "schedule_time": {"$lte": now},
            "status": "pending"
        })
        scheduled_jobs = await asyncio.to_thread(list, jobs_cursor)
        
        if not scheduled_jobs:
            return

        logger.info(f"Found {len(scheduled_jobs)} scheduled posts to process.")
        ...
        for job in scheduled_jobs:
            ...
            # Reconstruct a mock message object for context
            class MockMessage:
                def __init__(self, uid, cid, mid):
                    self.from_user = lambda: None
                    setattr(self.from_user, 'id', uid)
                    self.chat = lambda: None
                    setattr(self.chat, 'id', cid)
                    self.id = mid
            
            mock_msg = MockMessage(user_id, job.get("original_chat_id"), job.get("original_msg_id"))
            
            # Start the upload process
            logger.info(f"Starting scheduled upload for user {user_id}, job {job_id}.")
            task_tracker.create_task(
                safe_task_wrapper(process_and_upload(mock_msg, file_info, is_scheduled=True, job_id=job_id)),
                user_id=user_id,
                task_name=f"scheduled_upload_{job_id}"
            )
...
def run_server():
    try:
        ...
        logger.info(f"HTTP health check server started on port {server.server_port}.")
...
# === Main Entry Point ===
async def main():
    global mongo, db, valid_log_channel, upload_semaphore, MAX_FILE_SIZE_BYTES, MAX_CONCURRENT_UPLOADS

    os.makedirs("sessions", exist_ok=True)
    logger.info("Session directory ensured.")

    # --- Step 1: Database and Settings Setup ---
    logger.info("Attempting to connect to MongoDB...")
    try:
        mongo = MongoClient(MONGO_URI, serverSelectionTimeoutMS=5000)
        mongo.admin.command('ismaster')
        db = mongo.get_database("NowTok")
        logger.info("✅ Connected to MongoDB successfully.")

        logger.info("Loading global settings...")
        settings_from_db = db.settings.find_one({"_id": "global_settings"}) or {}
        
        # Apply defaults for any missing settings
        for key, value in DEFAULT_GLOBAL_SETTINGS.items():
            if key not in settings_from_db:
        ...
        db = None
        upload_semaphore = asyncio.Semaphore(DEFAULT_GLOBAL_SETTINGS["max_concurrent_uploads"])
        MAX_FILE_SIZE_BYTES = DEFAULT_GLOBAL_SETTINGS["max_file_size_mb"] * 1024 * 1024

    # --- Step 2: Start Health Check Thread ---
    server_thread = threading.Thread(target=run_server, daemon=True)
    server_thread.start()

    # --- Step 3: Start the Bot and Scheduler ---
    await app.start()
    logger.info("Pyrogram client started.")
    
    # Verify log channel after starting client
    if LOG_CHANNEL:
        try:
            await app.get_chat(LOG_CHANNEL)
            valid_log_channel = True
            logger.info(f"Successfully connected to log channel: {LOG_CHANNEL}")
            await send_log_to_channel(app, LOG_CHANNEL, "✅ **Bot has started successfully!**")
        except Exception as e:
            logger.error(f"Could not access LOG_CHANNEL ({LOG_CHANNEL}): {e}. Logging to channel disabled.")

    # Start the scheduler for background tasks
    if db:
        scheduler.add_job(check_scheduled_posts, 'interval', minutes=1, id="scheduled_post_checker")
        scheduler.start()
        logger.info("APScheduler started for scheduled posts.")
    else:
        logger.warning("Database not connected, scheduler for posts will not run.")
